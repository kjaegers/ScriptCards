...
Commands      : Note: Encounter Names are case sensitive.
    ...
    !eh update <Encounter Name>
        Does not require tokens to be selected. Updates the tokens for the given encounter by storing
...
const EncounterHelper = (() => {
    ...
    const tableCellStyle = 'text-align: center; margin: { top:1px, left:1px; }; padding:2px; color:white; background-color:#3c6478; font-size:x-small;';
    //const tableCellStyle = 'text-align: center; margin: { top:1px, left:1px; }; color:black; background-color:goldenrod; font-size:x-small;';
    const tableHeaderCellStyle = 'text-align: center; margin: { top:0px, left:0px; }; color:white; background-color:#093145; white-space:nowrap;';
    ...
    var translateList = {
        ...
        "include_drawings": {
            english: "Include drawing objects in encounters",
            french: "Inclure des objets de dessin dans les rencontres",
            spanish: "Incluir objetos de dibujo en encuentros",
            german: "Zeichnen Sie Objekte in Begegnungen ein"
        },
        "tokens_on_gm_layer": {
            english: "Note that your encounter has one or more tokens on the GM Layer which won't be automatically moved to the Object Layer.",
            french: "Notez que votre rencontre a un ou plusieurs jetons sur la couche GM qui ne seront pas automatiquement déplacés vers la couche d'objet.",
            spanish: "Tenga en cuenta que su encuentro tiene uno o más tokens en la Capa GM que no se moverán automáticamente a la Capa de Objeto.",
            german: "Beachten Sie, dass Ihr Aufeinandertreffen ein oder mehrere Tokens auf der GM-Ebene hat, die nicht automatisch auf die Objektebene verschoben werden."
        },
                ...
                switch (command.toLowerCase()) {
                    ...
                    case "!eh":
                        ...
                        switch (operation.toLowerCase()) {
                            case "ver":
                                sendChat(translate(APINAME), "/w gm Yep its the new version");
                                break;
                                    ...
                                    if (theToken !== undefined) {
                                        ...
                                        if (theToken !== undefined) {
                                            ...
                                            if (objtype == "t" && (theChar !== undefined)) {
                                                ...
                                                charName = 'Path/Drawing';
                                                columnDisplay = `<td style='${tableCellStyle}' colspan=${columns.length}&nbsp;</td>`;
                                            }
    ...
    function displayConfigMenu() {
        ...
        output += generateMenuToggle("include_drawings");
    ...
    function setEncounterLayer(pageid, encounterName, showing) {
                    ...
                    try {
                        ...
                        if (theToken !== undefined) {
                            mobinfo.forEach(function (info) {
                                ...
                                if (theToken.get(infoData[0]) !== undefined) {
                                    theToken.set(infoData[0], infoData[1].trim());
                                }
                            });
                        }
                    } catch {
                        log(`Error resetting token ${mob}`);
                    }
                }
            });
        }
    ...
    function getSavedLayer(mob) {
        var mobinfo = mob.split("#");
        var mobid = mobinfo[0];
        var returnLayer = "objects";
        mobinfo.shift();
        var theToken = getObj("graphic", mobid);
        if (theToken == undefined) {
            theToken = getObj("path", mobid);
